...
""" 
Vectorizer: text -> numerical data (TF-IDF)
...
2. transform (each description -> vector, each number represents the importance of a word), numerical value = TD-IDF score (term frequency (how often) * inverse document frequency (how rare))
...
def prepare_data(df, test_size = 0.2):
    ...
    #change for smaller dataset
    vectorizer = TfidfVectorizer(max_features=3000,
                                 ngram_range=(1, 2),
                                 min_df=3,
                                 max_df=0.9,
                                 sublinear_tf=True)     #max_features = 3000 - limit vocabulary to 3000 most used words
